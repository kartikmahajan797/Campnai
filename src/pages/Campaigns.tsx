...
const Campaigns = () => {
    ...
    const fetchInfluencers = async (currentPage: number, search: string = '') => {
        setIsLoading(true);
        try {
            const url = new URL(`${API_BASE_URL}/influencers`, window.location.origin);
    ...
    return (
        ...
        >
            <div className="flex flex-col items-center p-8 min-h-full">
                ...
                {/* Upload Card */}
                <div className="w-full max-w-2xl bg-card/50 border border-border rounded-2xl p-8 mb-8 backdrop-blur-sm">
                    <h2 className="text-2xl font-semibold text-foreground text-center mb-8">Upload Influencer List</h2>
                    ...
                    <div
                        ...
                        className={`border-2 border-dashed border-border rounded-xl p-8 flex flex-col items-center justify-center mb-6 bg-muted/30 hover:bg-muted/50 transition cursor-pointer ${isUploading ? 'opacity-50 pointer-events-none' : ''}`}
                    >
                        <div className="w-12 h-12 bg-muted rounded-lg flex items-center justify-center mb-4 text-primary">
                            {isUploading ? <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary"></div> : <FileText size={24} />}
                        </div>
                        <p className="text-muted-foreground mb-1">
                            {isUploading ? "Uploading..." : <>{'Drag & drop CSV or '}<span className="text-foreground underline">browse files</span></>}
                        </p>
                    </div>
                    ...
                    <div className="flex items-start justify-between gap-8">
                        <div className="text-sm text-muted-foreground">
                            ...
                            <ul className="list-disc pl-5 space-y-1 ml-1">
                                ...
                                <li>CONTACT NO.</li>
                                <li>EMAIL</li>
                                <li className="text-muted-foreground italic">...and more</li>
                            </ul>
                            <button
                                onClick={handleDownloadSample}
                                className="flex items-center gap-2 text-muted-foreground hover:text-foreground mt-4 text-xs transition"
                            >
                                <Download size={14} />
                                Download sample CSV
                            </button>
                        ...
                        <button
                            ...
                            className="bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-3 rounded-lg font-medium transition shadow-lg shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                ...
                <div className="w-full max-w-6xl mb-6 flex justify-between items-center gap-4">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input
                            ...
                            className="w-full bg-card/50 border border-border rounded-xl py-2.5 pl-10 pr-4 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 transition whitespace-nowrap overflow-hidden text-ellipsis"
                        />
                    </div>
                </div>

                {/* Table */}
                <div className="w-full max-w-6xl border border-border rounded-xl overflow-hidden bg-card/50 backdrop-blur-sm">
                    <div className="overflow-x-auto">
                        <Table>
                            <TableHeader className="bg-muted/50">
                                <TableRow className="border-border hover:bg-transparent whitespace-nowrap">
                                    <TableHead className="text-muted-foreground min-w-[150px]">Profile Link</TableHead>
                                    ...
                                    <TableHead className="text-muted-foreground min-w-[150px]">Contact No.</TableHead>
                            ...
                            <TableBody>
                                {isLoading ? (
                                    ...
                                    influencers.map((influencer, index) => (
                                        <TableRow key={influencer.id || index} className="border-border hover:bg-muted/50 whitespace-nowrap">
                                            <TableCell className="text-muted-foreground">
                                                {influencer.profile?.link ? (
                                                    <a href={influencer.profile.link} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
                                                        Visit Profile
                                                    </a>
                                                ) : '-'}
                                            </TableCell>
                                            ...
                                            <TableCell className="text-muted-foreground">{influencer.contact?.contact_no || '-'}</TableCell>
                                            <TableCell className="text-muted-foreground">{influencer.contact?.email || '-'}</TableCell>
                                        </TableRow>
                                    ))
                                )}
                            </TableBody>
                ...
                {totalPages > 1 && (
                    <div className="w-full max-w-4xl flex items-center justify-between mt-6 px-4">
                        <div className="text-sm text-muted-foreground">
                            Showing <span className="text-foreground">{(page - 1) * pageSize + 1}</span> to <span className="text-foreground">{Math.min(page * pageSize, totalCount)}</span> of <span className="text-foreground">{totalCount}</span> records
                        </div>
                        <div className="flex items-center gap-2">
                            <button
                                onClick={() => setPage(prev => Math.max(prev - 1, 1))}
                                disabled={page === 1}
                                className="px-4 py-2 text-sm bg-card border border-border rounded-lg text-foreground hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed transition"
                            >
                                Previous
                            </button>
                            <div className="flex items-center gap-1">
                                {[...Array(totalPages)].map((_, i) => (
                                    <button
                                        key={i + 1}
                                        onClick={() => setPage(i + 1)}
                                        className={`w-8 h-8 text-sm rounded-lg transition ${page === i + 1 ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`}
                                    >
                                        {i + 1}
                                    </button>
                                ))}
